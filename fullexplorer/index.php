...
if( $address === 'api' )
{
    ...
    if( $f === 'stats' )
    {
        ...
        if( $heightTime === false ) {
            ...
            $height = $heightTime[0];
            $timestamp = $heightTime[1];
        }
        
        $tx_query = $RO->db->db->query( 'SELECT r0 FROM pts ORDER BY r0 DESC LIMIT 1' )->fetchAll();
        $transactions = !empty( $tx_query ) ? (int)$tx_query[0][0] : 0;
        
        $addr_query = $RO->db->db->query( 'SELECT r0 FROM addresses ORDER BY r0 DESC LIMIT 1' )->fetchAll();
        $accounts = !empty( $addr_query ) ? (int)$addr_query[0][0] : 0;
        
        $blocks_query = $RO->db->db->query( 'SELECT r2 FROM hs ORDER BY r0 DESC LIMIT 100' )->fetchAll();
        if( count( $blocks_query ) > 1 ) {
            $newest_ts = (int)$blocks_query[0][0];
            $oldest_ts = (int)$blocks_query[count($blocks_query) - 1][0];
            $averageBlockTime = ( $newest_ts - $oldest_ts ) / ( count( $blocks_query ) - 1 );
        } else {
            $averageBlockTime = 6.0;
        }
        
        $assets_query = $RO->db->db->query( 'SELECT r0 FROM assets ORDER BY r0 DESC LIMIT 1' )->fetchAll();
        $assets = !empty( $assets_query ) ? (int)$assets_query[0][0] : 0;
        
        // Calculate the total staked (in satoshis) of all active Leases on-chain
        $staked_query = $RO->db->db->query( 'SELECT SUM(r6) FROM pts WHERE r2 = 8 AND r1 NOT IN (SELECT r9 FROM pts WHERE r2 = 9)' )->fetchAll();
        $totalStaked = !empty( $staked_query ) && isset($staked_query[0][0]) ? (int)$staked_query[0][0] : 0;
        
        // Calculate active generators/validators that generated blocks in the last 1472 blocks
        $generators = $RO->getGeneratorsFees( 1472, null );
        $activeValidators = count( $generators );
    ...
    if( $f === 'top' )
    {
        require_once 'include/RO.php';
        $RO = new RO( W8DB );
        $asset = $arg === false ? 'PLO' : $arg;
        $aid = ( $asset === 'Waves' || $asset === 'PLO' || $asset === 'WAVES' ) ? 0 : $RO->getIdByAsset( $asset );
        if( $aid === false )
            apiexit( 404, [ 'code' => 404, 'message' => 'asset not found' ] );
        
        $info = $RO->getAssetInfoById( $aid );
        $decimals = $aid === 0 ? 8 : ( ( $info[0] === 'N' ) ? 0 : (int)$info[0] );
        $assetName = $aid === 0 ? 'PLO' : substr( $info, 2 );

        $limit = 100;
        $offset = $arg2 === false ? 0 : (int)$arg2;
        
        $balances = $RO->db->query( 'SELECT * FROM balances WHERE r2 = ' . $aid . ' ORDER BY r3 DESC LIMIT ' . $limit . ' OFFSET ' . $offset )->fetchAll();
        $json = [];
        $rank = $offset + 1;
        foreach( $balances as $balance )
        {
            ...
            if( $amount <= 0 )
                break;
            $addrId = (int)$balance[1];
            $address = $RO->getAddressById( $addrId );
            $json[] = [
                'rank' => $rank++,
                'address' => $address,
                'balance' => $amount,
                'decimals' => $decimals,
                'assetName' => $assetName,
                'assetId' => $asset === 'PLO' ? 'PLO' : $asset
            ];
        }

        $totalHolders = (int)$RO->db->query( 'SELECT COUNT(*) FROM balances WHERE r2 = ' . $aid . ' AND r3 > 0' )->fetchColumn();
    ...
    if( $f === 'generators' )
    {
        require_once 'include/RO.php';
        $RO = new RO( W8DB );
        $limit = $arg === false ? 1472 : intval($arg);
        $generators = $RO->getGeneratorsFees( $limit, null );
        
        $json = [];
        foreach( $generators as $gen => $data )
        {
            $address = $RO->getAddressById( $gen );
            $json[] = [
                'address' => $address,
                'blocks' => count( $data ),
                'fees' => array_sum( $data )
    ...
    if( $f === 'transactions' )
    {
        ...
        $limit = 100;
        $offset = $arg === false ? false : (int)$arg;

        if( $offset !== false )
            $q = $RO->db->query( "SELECT r0 FROM ts WHERE r0 <= $offset ORDER BY r0 DESC LIMIT " . $limit )->fetchAll();
        else
            $q = $RO->db->query( "SELECT r0 FROM ts ORDER BY r0 DESC LIMIT " . $limit )->fetchAll();
        ...
        if( !empty( $txkeys ) )
        {
            $in_clause = implode( ',', $txkeys );
            $pts_rows = $RO->db->query( "SELECT * FROM pts WHERE r1 IN ($in_clause) ORDER BY r0 DESC" )->fetchAll();
            ...
            foreach( $txkeys as $tk )
            {
                ...
                $txid = $RO->getTxIdByTxKey( $tk );
                $height = w8k2h( $tk );
                $timestamp = $RO->getTimestampByHeight( $height );

                $sender = $RO->getAddressById( (int)$primary[3] );
                $recipient = $RO->getAddressById( (int)$primary[4] );

                if( $sender === 'SELF' ) $sender = '';
                if( $recipient === 'SELF' ) $recipient = '';

                $asset_id = (int)$primary[5];
                if( $asset_id === 0 || $asset_id === -1 )
                    $asset_str = 'PLO';
                else
                {
                    $asset_str = $RO->getAssetById( $asset_id );
                    if( $asset_str === false ) $asset_str = (string)$asset_id;
                }

                $fee_asset_id = (int)$primary[7];
                if( $fee_asset_id === 0 || $fee_asset_id === -1 )
                    $fee_asset_str = 'PLO';
                else
                {
                    $fee_asset_str = $RO->getAssetById( $fee_asset_id );
                    if( $fee_asset_str === false ) $fee_asset_str = (string)$fee_asset_id;
                }
                ...
                $json[] = [
                    ...
                    'sender' => $sender,
                    'recipient' => $recipient,
                    'amount' => (int)$primary[6],
                    'fee' => (int)$primary[8],
                    'feeAsset' => $fee_asset_str,
                    'height' => $height,
                    'assetId' => $asset_str
                ];
            }
    ...
    if( $f === 'contracts' )
    {
        ...
        if( $offset !== false )
            $q = $RO->db->query( "SELECT * FROM (SELECT r3, MAX(r0) as max_r0, r1 FROM pts WHERE r2 = 13 GROUP BY r3) WHERE max_r0 <= $offset ORDER BY max_r0 DESC LIMIT " . $limit )->fetchAll();
        else
            $q = $RO->db->query( "SELECT * FROM (SELECT r3, MAX(r0) as max_r0, r1 FROM pts WHERE r2 = 13 GROUP BY r3) ORDER BY max_r0 DESC LIMIT " . $limit )->fetchAll();
        ...
        foreach( $q as $row )
        {
            ...
            $contract_id = (int)$row['r3'];
            $contract_address = $RO->getAddressById( $contract_id );

            // Count InvokeScript (type 16) calls to this contract
            $call_q = $RO->db->query( "SELECT COUNT(*) FROM pts WHERE r2 = 16 AND r4 = $contract_id" )->fetchAll();
            $call_count = !empty($call_q) ? (int)$call_q[0][0] : 0;

            $json[] = [
                'address' => $contract_address,
                'name' => 'Contract_' . substr($contract_address, -6),
                'description' => 'Active Smart Contract / dApp on Planet One',
                'complexity' => 120,
                'type' => 'dApp',
    ...
    if( $f === 'assets' && $arg === 'details' && $arg2 !== false )
    {
        $wk = wk();
        $res = $wk->fetch( "/assets/details/" . $arg2 );
        if( $res === false )
        {
            apiexit( 404, [ 'code' => 404, 'message' => 'asset not found on node' ] );
        }
...
if( $address === 'top' && $f !== false )
{
    ...
    $aid = ( $f === 'Waves' || $f === 'PLO' || $f === 'WAVES' ) ? 0 : $RO->getIdByAsset( $f );
    if( $aid !== false )
    {
        $info = $RO->getAssetInfoById( $aid );
        if( $info !== false )
...
function prolog()
{
    ...
    $L = (int)( $_COOKIE['L'] ?? 0 ) === 1;
    $displayAddress = prettyAddress( $address );
    $title = 'Planet One FullExplorer &#183; ' . $displayAddress;
    if( $f !== false &&
        ( $address !== 'GENERATORS' || $f !== 1472 ) &&
        ( is_numeric( $f ) || strlen( $f ) >= 32 || $f === 'Waves' || $f === 'PLO' || $f === 'WAVES' ) )
    {
        $displayF = ( $f === 'Waves' || $f === 'WAVES' ) ? 'PLO' : prettyAddress( $f );
        $title .= ' &#183; ' . $displayF;
    }
    echo sprintf( '
<!DOCTYPE html>
<html>
    ...
    <body>
        <!-- Planet One Header -->
        <header class="planetone-header">
            <div class="header-container">
                <a href="/" class="brand-link">
                    <img src="/static/logo.png" alt="Planet One Logo" class="brand-logo">
                    <span class="brand-name">PLANET<span class="highlight">ONE</span> <span class="sub-brand">FullExplorer</span></span>
                </a>
        ...
        <pre class="monospace-content">
', 
    $title, 
    $L ? '-l' : '-n',
    ( $address !== 'GENERATORS' && $address !== 'ACTIVATION' && $address !== 'tx' && $address !== 'top' ) ? htmlspecialchars( $address ) : '',
    $L ? '&#9680; Dark Theme' : '&#9681; Light Theme' 
    );
}
...
function w8io_print_transactions( $aid, $where, $uid, $count, $address, $d, $summary = false )
{
            ...
            foreach( $changes as $a => $diffs )
            {
                ...
                foreach( $diffs as $asset => $amount )
                {
                    ...
                    else if( $asset === 0 )
                    {
                        ...
                        $asset = ' <a href="' . W8IO_ROOT . $address . '/f/PLO">PLO</a>';
                    }
                    else if( $asset === WAVES_LEASE_ASSET )
                    {
                        $sign = $amount < 0 ? -1 : 1;
                        $amount = ' ' . w8io_sign( $sign ) . w8io_amount( $amount, 8, 0, false );
                        $asset = ' <a href="' . W8IO_ROOT . $address . '/f/PLO">PLO (GENERATOR)</a>';
                    }
                    else
                        continue;
                    ...
                    else
                    {
                        $last_a = $a;
                        $len_a = strlen( $a );
                        if( $len_a >= 34 && $len_a <= 36 )
                            $diff .= w8io_a( $a ) . ': ' . $amount . $asset . PHP_EOL;
                        else
                            $diff .= '<span>' . str_repeat( '—', max( 0, 34 - $len_a ) ) . ' </span>' . w8io_a( $a ) . ': ' . $amount . $asset . PHP_EOL;
                    }
                }
            }
    ...
    foreach( $pts as $ts )
    {
        ...
        if( $isa && $fee )
        {
            ...
            {
                $fee = ' <small>' . w8io_amount( $fee, 8, 0 ) . ' <a href="' . W8IO_ROOT . $address . '/f/PLO">PLO</a></small>';
            }
        }
        else
            $fee = '';
...
function is_address( $value )
{
    if( !is_string( $value ) )
        return false;
    $len = strlen( $value );
    return $len >= 34 && $len <= 36 && preg_match( '/^[1-9A-HJ-NP-Za-km-z]+$/', $value ) === 1;
}
...
else
{
    ...
    if( $arg !== false )
    {
        ...
        {
            {
                if( $arg === 'Waves' || $arg === 'PLO' || $arg === 'WAVES' )
                    $arg = 0;
                else
                if( is_numeric( $arg ) )
                {
            ...
            if( is_numeric( $arg ) )
            {
                ...
                else
                {
                    ...
                    if( $first === '>' || $first === '<' )
                    {
                        ...
                        $asset1 = $asset1 === WAVES_ASSET ? 'PLO' : $RO->getAssetById( $asset1 );
                        $asset2 = $asset2 === WAVES_ASSET ? 'PLO' : $RO->getAssetById( $asset2 );
    ...
    else
    {
        ...
        if( $aid <= 0 || $len_address < 34 || $len_address > 36 )
        ...
        // WAVES
        {
            ...
            $furl = W8IO_ROOT . $address . '/f/PLO';

            if( $arg === 0 && $filter === 1 )
            {
                echo '<b>' . $amount . ' <a href="' . W8IO_ROOT . 'top/PLO">' . $asset . '</a></b>';
                echo ' <small><a href="' . W8IO_ROOT . $address . '/fi/PLO">i</a><a href="' . W8IO_ROOT . $address . '/fo/PLO">o</a></small>' . PHP_EOL;
        ...
        if( $f === 'pay' )
        {
            ...
            for( ;; )
            {
                ...
                echo w8io_amount( $waves_fees, 8 ) . " PLO" . PHP_EOL;
...
echo "Planet One Block Explorer &copy; " . date('Y') . " | Powered by <a href=\"https://github.com/D-H-O-R-A/planetone-explorer\" target=\"_blank\">Better2better</a>";
if( file_exists( '.git/FETCH_HEAD' ) )
{
    $rev = file_get_contents( '.git/FETCH_HEAD', false, null, 0, 40 );
    echo "/<a href=\"https://github.com/D-H-O-R-A/planetone-explorer/commit/$rev\" target=\"_blank\">" . substr( $rev, 0, 7 ) . '</a> ';
}
if( !isset( $showtime ) )
{
